...
const auth = { currentUser: { uid: 'local_user', email: 'local@example.com' } };
const onAuthStateChanged = (_, cb) => cb({ uid: 'local_user' }); // Auto-login as mock user
...
savePublishBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    console.log("=== Exam Data Saved Locally ===");
    console.log(exam);
    console.log("===============================");

    alert("تم حفظ الاختبار مجليًا (Prototype)\nراجع Console للتفاصيل.");
